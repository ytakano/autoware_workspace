#!/usr/bin/env python3
"""POT/GPD extreme-value analysis of the Isolated warm samples (roadmap C2, stdlib-only).
...
def main():
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Per-session POT/GPD tail diagnostics ($n{=}1000$/session; 95\% threshold;"
        r" \evtBoot{} bootstrap draws). Dependence invalidates extrapolation; values are"
        r" retained only to diagnose the rejection. Times in \si{ms}.}",
        r"\label{tab:evt}",
        r"\footnotesize",
